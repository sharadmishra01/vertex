...
import demo.discovery.util.Runner;
...
//	}
	...
	public static String DISCOVERY_HOST = "192.168.56.1";
	
	public static final String DISCOVERY_CLIENT = "192.168.99.100";
	...
	public void start() throws Exception {
		...
		System.out.println(" Current instance of Demo Service is " + this.toString());
		mongo = MongoClient.createShared(vertx,
				new JsonObject().put("db_name", "test").put("host", DISCOVERY_CLIENT));
		...
		router.get("/api/disco").handler(
				ctx -> {
					System.out.println(" I came to list current discovery results" + this.toString());
		...
		router.post("/api/discovery")
				.handler(ctx -> {
					System.out.println(" I came to start discovery " + this.toString());
					...
					// ctx.setUser(newUser);
						...
						try {
							StartDiscoveryUtils.sendStartDiscoveryData(vertx,
									...
									DEFAULT_SERVER_PORT,
									(Inet4Address) Inet4Address
											.getByName(DISCOVERY_HOST), // change
																		// the
																		// source
																		// and
																		// target
